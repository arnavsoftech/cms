...
class Menu extends MY_Controller{
	...
	function __construct(){
		parent::__construct();
		$this -> load -> model(array('Menu_model', 'Category_model', 'Post_model'));
	...
	public function group($id = false){
		...
		$this -> data['menu'] = $this -> Menu_model -> getNew('ai_menugroup');
		if($id){
			$this -> data['menu'] = $this -> Menu_model -> getRow($id, 'ai_menugroup');
		}
    ...
    function settings(){
        $this -> data['main'] = admin_view('menu/settings');
        if($this -> input -> post('submit')){
            ...
            if($menus <> NULL){
                foreach($menus as $m){
                    $op = array();
                    $op['option_name'] = $m[0];
                    $op['option_value'] = $this -> input -> post($m[0]);
                    $this -> Setting_model -> save_option($op);
                }
            }
        }

        $this -> data['groups'] = $this -> Menu_model -> all_groups();
        $this -> load -> view(admin_view('default'), $this -> data);
    }
}
